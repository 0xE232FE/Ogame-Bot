...
from ogbot.scraping import general, galaxy
from ogbot.scraping.scraper import Scraper
        ...
        for page in range(1, page_count):
            ...
            try:
                ...
                data = urllib.urlencode(
                    {'messageId': -1, 'tabid': 20, 'action': 107, 'pagination': page_number, 'ajax': 1})
                res = self.open_url(url, data)
                soup = BeautifulSoup(res.read(), "lxml")
        ...
        for message_box in message_boxes:
            ...
            try:
                ...
                if is_spy_report:
                    ...
                    report = SpyReport(planet_name.encode('utf-8'), player_name, player_state, coordinates, resources,
                                       fleet,
...
def parse_resource(text):
    ...
    try:
        value = int(
            text.split(':')[1].strip().replace(".", "").replace(",", "").replace("Mn", "000").replace("M", "000"))
    except Exception as e:
